...
Dictionarytype::Dictionarytype(QWidget *parent)
	: QWidget(parent)
{
	ui.setupUi(this);
...
void Dictionarytype::initUI()
{
	ui.treeWidget->clear();
	ui.tableWidget->setRowCount(0);
	QStringList headers;
	headers << QString::fromLocal8Bit("字典类型");
	...
	QString strsql;
	strsql = "select * from sys_dictionarytype order by id";
...
void Dictionarytype::showTable(QTreeWidgetItem*item, int column)
{
	ui.tableWidget->setRowCount(0);
	QString strText = item->text(0);
	QString strsql;
	strsql = "select * from sys_dictionarytype where name  = '"+strText+"'  order by id";
	QSqlQuery query(*sql.db);	
	query.exec(strsql);
	int row = 0;
	while(query.next())
...
void Dictionarytype::on_addButton_clicked()
{
	initUI();
...
void Dictionarytype::on_editButton_clicked()
{
	ui.tableWidget->setEditTriggers(QAbstractItemView::AllEditTriggers);
	ui.saveButton->setEnabled(true);
...
void Dictionarytype::on_saveButton_clicked()
{

	QSqlQuery query(*sql.db);		
	QString strsql = "delete from sys_dictionarytype"; 
	query.exec(strsql);
	int row = ui.tableWidget->model()->rowCount();
	for (int i= 0;i<row;i++)
	{
		query.prepare("INSERT INTO sys_dictionarytype VALUES (?,?, ?, ?, ?)");
...
void Dictionarytype::on_deleteButton_clicked()
{
	...
	if(box.exec()==QMessageBox::Ok)
	{
		...
		QSqlQuery query(*sql.db);
		query.exec("delete  from sys_dictionarytype where name = '"+strName+"'");
		initUI();
	}
}
void Dictionarytype::on_exitButton_clicked()
{
