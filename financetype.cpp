...
Financetype::Financetype(QWidget *parent)
	: QWidget(parent)
{
	ui.setupUi(this);
...
void Financetype::initUI()
{
	...
	QStringList headers;
	headers << QString::fromLocal8Bit("财务分类");
	...
	QString strsql;
	strsql = "select * from sys_financetype order by id";
...
void Financetype::showTable(QTreeWidgetItem*item, int column)
{
	ui.tableWidget->setRowCount(0);
	QString strText = item->text(0);
	QString strsql;
	strsql = "select * from sys_financetype where name  = '"+strText+"'  order by id";
	QSqlQuery query(*sql.db);	
	query.exec(strsql);
	int row = 0;
	while(query.next())
...
void Financetype::on_addButton_clicked()
{
	initUI();
...
void Financetype::on_editButton_clicked()
{
	ui.tableWidget->setEditTriggers(QAbstractItemView::AllEditTriggers);
	ui.saveButton->setEnabled(true);
...
void Financetype::on_saveButton_clicked()
{
	...
	QSqlQuery query(*sql.db);		
	QString strsql = "delete from sys_financetype"; 
	query.exec(strsql);
	int row = ui.tableWidget->model()->rowCount();
	for (int i= 0;i<row;i++)
	{

		query.prepare("INSERT INTO sys_financetype VALUES (?,?, ?, ?, ?)");
...
void Financetype::on_exitButton_clicked()
{
